...
\item Performance counter measurements were performed on the XSBench mini-app,
  ...
  to mimic, OpenMC.
  %RR \item The previous study presented results on a multiprocessor architecture.  However, logical threads were not explicitly mapped to physical processors, and effects from NUMA could not exposed.
\end{itemize}
...
memory.
%RR In many architectures with multiple CPUs, main memory is further subdivided between CPUs; and in some multicore CPUs, L3 is similarly subdivided between cores.  In both cases, each processing element has access to both its own local memory and the remote memory from other processing elements; but local memory can be accessed with siginificantly less overhead from latency and contention.  These designs implement \emph{non-uniform uniform memory access (NUMA)}, and applications running on NUMA architectures benefit greatly from ``NUMA-aware'' memory access patterns.  For example, an application can suffer from less latency if each parallel task can its restrict memory accessses to its local memory.
...
%RR (Mentioning TLB may be TMI, since it's never referred to in your results or discussion.)
